...
Technical repository behind **"Creating the missing data"** — an open, reproducible
pipeline that turns OpenStreetMap into a dataset of **sports facilities across
Africa**, then maps and analyzes it.

There is no consolidated open dataset of African sports infrastructure. This repo
builds one from OSM (imperfect, non-representative, but open and *re-runnable*), and
...
> **195** distinct sport tags · 60,794 football pitches. Figures are regenerated by
...
`data/sports_facilities.csv` — one row per OSM feature carrying a `sport=*` tag:
