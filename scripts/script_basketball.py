...
# Filtrer uniquement les sites où sport=basketball (y compris s'il y a plusieurs sports, ex: "netball;basketball")
df = df[df["sport"].str.contains("basketball", na=False, case=False)]
...
print(f"\n✅ Nombre total de sites pour basketball : {len(df)}")
print(df.head())

# Créer une carte centrée sur l'Afrique
m = folium.Map(location=[0, 25], zoom_start=3)

# Ajouter un point pour chaque site de basketball
...
m.save("basketball_map.html")
print("\n✅ Carte générée : ouvre 'basketball_map.html' dans un navigateur.")
